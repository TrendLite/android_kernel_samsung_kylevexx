...
#include <linux/gpio.h> //SPK -> LOUDMIC
...
#ifdef CONFIG_LOUDMIC_FEATURE
	aTrace(LOG_AUDIO_DRIVER,  "SPEAKER -> LOUDMIC AUDDRV_Telephony_Init - %s mic=%d, spkr=%d \n", __func__, mic, speaker);
	if ((currVoiceSpkr == AUDIO_SINK_LOUDSPK) || ((currVoiceSpkr == AUDIO_SINK_HANDSET)&& gFactory_submic)){ //GPIO_START
		aTrace(LOG_AUDIO_DRIVER,  "Loud_MIC GPIO \n");
		gpio_request(GPIO186_MIC_SELECT, "Loud_MIC enable");
		gpio_val = gpio_get_value(GPIO186_MIC_SELECT);
		aTrace(LOG_AUDIO_DRIVER,  "Loud_MIC GPIO SPEAKER -> LOUDMIC gpio_val=%d, currVoiceSpkr=%d\n", gpio_val, currVoiceSpkr);
		gpio_direction_output(GPIO186_MIC_SELECT, Loud_MIC);
		gpio_free(GPIO186_MIC_SELECT);
	} else { //GPIO_STOP
		aTrace(LOG_AUDIO_DRIVER,  "Main_MIC GPIO \n");
		gpio_request(GPIO186_MIC_SELECT, "Main_MIC enable");
		gpio_val = gpio_get_value(GPIO186_MIC_SELECT);
		aTrace(LOG_AUDIO_DRIVER,  "Main_MIC GPIO SPEAKER -> MAINMIC gpio_val=%d, currVoiceSpkr=%d\n", gpio_val, currVoiceSpkr);
	...
	if (userEQOn == FALSE) {
		/* Use the old code, before CP function
		   ...
		   handle the mode properly.*/
			...
			audio_mode + audio_app * AUDIO_MODE_NUMBER, 7, 0, 0, 0);
		/*
		audio_control_generic(AUDDRV_CPCMD_SetFilter,
			...
			for (j = 0; j < MAX_BLOCK_NUM; j++)
				if (path->srcmRoute[i][j].outChnl & outChnl_select) {
					...
					aTrace(LOG_AUDIO_DRIVER, "%s pathID %d outChnl 0x%x inChnl 0x%x\n",
						__func__, param.pathID, outChnl, path->srcmRoute[i][j].inChnl);
		...
		for (i = 0; i < MAX_SINK_NUM; i++)
			for (j = 0; j < MAX_BLOCK_NUM; j++)
				...
				if (path->srcmRoute[i][j].outChnl & outChnl_select) {
					...
					aTrace(LOG_AUDIO_DRIVER, "%s pathID %d found outChnl 0x%x inChnl 0x%x\n",
						__func__, param.pathID, outChnl, path->srcmRoute[i][j].inChnl);
...
*
* Description:   Get Telephony path
*
*****************************************************************************/
AUDDRV_PathID_t *AUDDRV_GetTelephonyPath(void)
{
	aTrace(LOG_AUDIO_DRIVER,  "%s (ul: %d, ul2: %d, dl: %d)\n", __func__,
		telephonyPathID.ulPathID, telephonyPathID.ul2PathID, telephonyPathID.dlPathID);

	return &telephonyPathID;
}
