...
static bool isHwCfgDspMute = FALSE;
/* for CP reset */
...
static int wait_fm_hspmu_on = 40*1000;
static int wait_fm_ihfpmu_on = 50*1000;
...
void AUDCTRL_EnableTelephony(AUDIO_SOURCE_Enum_t source, AUDIO_SINK_Enum_t sink)
{
	...
	// it's necessary to tune AUDIO_MODE_USB with HEADPHONE.
	if ((sink == AUDIO_SINK_HEADSET) &&
		(source == AUDIO_SOURCE_ANALOG_MAIN)) {
		mode = AUDIO_MODE_USB;
		aTrace(LOG_AUDIO_CNTLR, "%s sink %d - mode %d\n", __func__, sink, mode);
	}
...
#ifdef CONFIG_AUDIO_FEATURE_SET_DISABLE_ECNS
		if ((pre_app == app) &&
			(mode == AUDIO_MODE_BLUETOOTH || mode == AUDIO_MODE_HANDSFREE))
#else
...
#endif
	} // run when isHwCfgDspMute is false
}
...
#ifdef CONFIG_BCM_MODEM
		RPC_SetProperty(RPC_PROP_AUDIO_MODE,
				(UInt32) (((int)app * AUDIO_MODE_NUMBER + mode)));
#endif
	}
			...
			unsigned int *pPathID)
{
	...
	if (source == AUDIO_SOURCE_I2S && AUDCTRL_InVoiceCall() == FALSE) {
		...
		AUDCTRL_ConfigWait(AUDCTRL_WAIT_HSPMU_ON, wait_fm_hspmu_on);
...
*/
	{
		if (source == AUDIO_SOURCE_I2S
			...
			AUDCTRL_ConfigWait(AUDCTRL_WAIT_PMU_OFF, wait_fm_pmu_off);
			powerOnExternalAmp(sink, FmUse, FALSE, FALSE);
			AUDCTRL_ConfigWait(AUDCTRL_WAIT_PMU_OFF, -1);
			fmPlayStarted = FALSE;
	...
	***/
	...
	if (gain_format == AUDIO_GAIN_FORMAT_FM_RADIO_DIGITAL_VOLUME_TABLE) {
		...
		if (mixInCh != CSL_CAPH_SRCM_INCHNL_NONE) {
			...
			mixInGain_r = users_gain[AUDPATH_FM].R;

			aTrace(LOG_AUDIO_CNTLR,
				"%s users_gain.valid %d users_gain.L %d users_gain.R %d\n",
				__func__, users_gain[AUDPATH_FM].valid,
				users_gain[AUDPATH_FM].L, users_gain[AUDPATH_FM].R);
			   ...
			   AUDIO_SINK_Enum_t sink, unsigned int pathID)
{
	...
	if (source == AUDIO_SOURCE_I2S && AUDCTRL_InVoiceCall() == FALSE
		...
		{
			AUDCTRL_ConfigWait(AUDCTRL_WAIT_HSPMU_ON, wait_fm_hspmu_on);
			AUDCTRL_ConfigWait(AUDCTRL_WAIT_IHFPMU_ON, wait_fm_ihfpmu_on);
			powerOnExternalAmp(sink, FmUse, TRUE, FALSE);
			AUDCTRL_ConfigWait(AUDCTRL_WAIT_HSPMU_ON, -1);
			AUDCTRL_ConfigWait(AUDCTRL_WAIT_IHFPMU_ON, -1);
		}
...
{
	aTrace(LOG_AUDIO_CNTLR, "%s mode = %d (ref. %d)\n",
			__func__, mode, ((mode == BT_MODE_WB) || (mode == BT_MODE_WB_TEST)));
			    ...
			    int arg1, int arg2, int arg3, int arg4)
{
	...
	case AUDCTRL_HW_CFG_DSPMUTE:
		AUDCTRL_SetTelephonySpkrMute(AUDIO_SINK_UNDEFINED, (bool)arg1);
		isHwCfgDspMute = (bool)arg1;
	...
	case AUDCTRL_HW_CFG_DOCKING:
		IsAudioDock = arg1;
		aTrace(LOG_AUDIO_CNTLR,"IsAudioDock=%d\n",IsAudioDock);
			       ...
			       int use, int force)
{
	...
	if (force == TRUE) {
		...
		if (use == TRUE) {
			if (IHF_IsOn == TRUE) {
				...
				extern_ihf_on();
#endif
				setExternAudioGain(GetAudioModeBySink(AUDIO_SINK_LOUDSPK),
						   AUDCTRL_GetAudioApp());
			}
			if (HS_IsOn == TRUE) {
				extern_hs_on();
				setExternAudioGain(GetAudioModeBySink(AUDIO_SINK_HEADSET),
						   AUDCTRL_GetAudioApp());
			}
			ampControl = TRUE;
		}
	...
	if ((callUseHS == FALSE) && (audioUseHS == FALSE)
		...
		if (HS_IsOn != FALSE) {
			...
			audctl_usleep_range(wait_before_pmu_off, wait_before_pmu_off+2000);
			extern_hs_off();
/*			audctl_usleep_range(20000,
		...
		if (HS_IsOn != TRUE && ampControl == TRUE) {
			aTrace(LOG_AUDIO_CNTLR, "powerOnExternalAmp power on HS");
			audioh_start_hs();

			audctl_usleep_range(wait_hspmu_on, wait_hspmu_on+2000);
			extern_hs_on();
			audctl_usleep_range(wait_hspmu_on, wait_hspmu_on+2000);
				}
	...
	if ((callUseIHF == FALSE) && (audioUseIHF == FALSE)
		...
		if (IHF_IsOn != TRUE && ampControl == TRUE) {
			...
			audctl_usleep_range(wait_ihfpmu_on, wait_ihfpmu_on + 2000);
#if defined(CONFIG_IHF_EXT_PA_TPA2026D2)
			if (bInVoiceCall)
				tpa2026d2_spk_power(1, 0);
			else
...
{
	static int old_wait[AUDCTRL_WAIT_TOTAL] = {0};
	int *p_wait = NULL, new_wait = wait_length;
	...
	if (p_wait) {
		if (new_wait >= 0) {
			old_wait[wait_id] = *p_wait;
		*p_wait = new_wait;
		} else {
			*p_wait = old_wait[wait_id];
		}
	} else {
		aTrace(LOG_AUDIO_CNTLR, "%s cannot set delay with wait id %d\n",
			__func__, wait_id);
		return;
	}

	aTrace(LOG_AUDIO_CNTLR, "%s wait id %d length %d to %d usec\n",
		__func__, wait_id, old_wait[wait_id], *p_wait);
}
